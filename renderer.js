...
async function listSerialPorts() {
	try {
		...
		if (adafruitPort && adafruitPort.path) {
			...
			document.getElementById('error').textContent =
				'Adafruit port found but path is undefined';
		} else {
			document.getElementById('error').textContent =
				'Adafruit port not found';
		}
	} catch (err) {
		document.getElementById('error').textContent =
			'Failed to list serial ports: ' + err.message;
	}
}
